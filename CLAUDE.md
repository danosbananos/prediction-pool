...
- **Fighter data:** Glory Kickboxing API (primary) + Wikipedia/Wikidata (fallback) — see `fighter_lookup.py`
...
- **Glory API as primary data source:** The Statamic API at `glory-api.pinkyellow.computer` provides images, records, nationality, and flags for all Glory fighters. Wikipedia/Wikidata is the fallback for fighters not in the Glory database. The Glory API can be slow (20s timeout configured).
...
| File | Purpose |
|------|---------|
| `app.py` | Main app: models (Pool, Match, Participant, Prediction), all routes, helpers |
